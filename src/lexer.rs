    ...
    T: Iterator<Item = char>,
{
    ...
    fn lex_identifier(&mut self) -> LexResult {
        ...
        loop {
            // Detect r"", f"", b"" and u""
            if !(saw_b || saw_u) && (self.chr0 == Some('b') || self.chr0 == Some('B')) {
                saw_b = true;
            } else if !(saw_b || saw_r || saw_u)
            ...
            if self.chr0 == Some('"') || self.chr0 == Some('\'') {
                return self.lex_string(saw_b, saw_r, saw_u);
            }
        }
    ...
    fn lex_normal_number(&mut self) -> LexResult {
        ...
        if self.chr0 == Some('.') || self.at_exponent() {
            ...
            let end_pos = self.get_pos();
            let value = value_text.parse::<u64>().unwrap();
            ...
            Ok((start_pos, Tok::UInt { value }, end_pos))
        }
    }
    ...
    fn lex_string(&mut self, is_bytes: bool, is_raw: bool, _is_unicode: bool) -> LexResult {
    ...
    fn consume_character(&mut self, c: char) -> Result<(), LexicalError> {
        match c {
            ...
            '#' => {
                self.eat_single_char(Tok::Number);
            }
            '"' => {
                let string = self.lex_string(false, false, false)?;
                self.emit(string);
            }
            ',' => {
                let tok_start = self.get_pos();
            ...
            '[' => {
                self.eat_single_char(Tok::Lbracket);
                self.nesting += 1;
            }
            ']' => {
                self.eat_single_char(Tok::Rbracket);
                if self.nesting == 0 {
                    return Err(LexicalError {
                        error: LexicalErrorType::NestingError,
                        location: self.get_pos(),
            ...
            '\'' => {
                self.eat_single_char(Tok::Of);
            }
            '+' => {
                self.eat_single_char(Tok::Add);
            }
            '-' => {
                let tok_start = self.get_pos();
                self.next_char();
                if let Some('-') = self.chr0 {
                    self.next_char();
                    let tok_end = self.get_pos();
                    self.emit((tok_start, Tok::Seq, tok_end));
                } else {
                    let tok_end = self.get_pos();
                    self.emit((tok_start, Tok::Sub, tok_end));
                }
            }
            '*' => {
                let tok_start = self.get_pos();
                ...
                if let Some('*') = self.chr0 {
                    self.next_char();
                    let tok_end = self.get_pos();
                    self.emit((tok_start, Tok::Pow, tok_end));
                } else {
                    let tok_end = self.get_pos();
                    self.emit((tok_start, Tok::Mul, tok_end));
                }
            }
            '/' => {
                let tok_start = self.get_pos();
                ...
                if let Some('/') = self.chr0 {
                    ...
                    let tok_end = self.get_pos();
                    self.emit((tok_start, Tok::Div, tok_end));
                }
            }
            _ => {
